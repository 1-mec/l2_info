...
void gere_client(int accept_sd, struct sockaddr_in addrcli, int len_addrcli) {
    ...
    struct enchere* meilleure_enchere = (struct enchere *)shmat(id_shm, NULL, 0);
	...
	printf("Login de %s\n", nom);
	while (actif) {
		ret = sendto(accept_sd, msg_bienvenue2, strlen(msg_bienvenue2), 0,
					 (const struct sockaddr*)&addrcli, len_addrcli);
		printf("sendto = %d\n", ret);
		if (ret == -1) {
			err("sendto");
		...
		if (ret == -1) {
			...
			if (montant > meilleure_enchere->montant) {
				snprintf(replymsg, maxlength,
						 "Votre enchere de %0.2f est retenue\n", montant);
				meilleure_enchere->montant = montant;
			} else {
				snprintf(replymsg, maxlength,
